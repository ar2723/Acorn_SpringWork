...
-- 직업 게시판 게시글을 저장할 테이블 
CREATE TABLE board_cafe_class(
    num NUMBER PRIMARY KEY, --글번호
    className VARCHAR2(100) NOT NULL, -- 직업군
    writer VARCHAR2(100) NOT NULL, --작성자 (로그인된 아이디)
    title VARCHAR2(100) NOT NULL, --제목
    content CLOB, --글 내용
    viewCount NUMBER, -- 조회수
...
-- 직업 게시판 게시글의 번호를 얻어낼 시퀀스
CREATE SEQUENCE board_cafe_class_seq NOCACHE; 

-- 직업 게시판 댓글을 저장할 테이블
CREATE TABLE board_cafe_class_comment(
    num NUMBER PRIMARY KEY, --댓글의 글번호
    writer VARCHAR2(100), --댓글 작성자의 아이디
    content VARCHAR2(500), --댓글 내용
    target_id VARCHAR2(100), --댓글의 대상자 아이디
...
-- 직업 게시판 댓글의 글번호를 얻어낼 시퀀스
CREATE SEQUENCE board_cafe_class_comment_seq NOCACHE;
